...
def create_parser() -> argparse.ArgumentParser:
    ...
    skills_list.add_argument('--show-disabled', action='store_true', help='Include disabled skills')

    skills_info = skills_subparsers.add_parser('info', help='Show detailed skill information')
    ...
    # Patterns command
    patterns_parser = subparsers.add_parser('patterns', help='Browse and search patterns')
    ...
    patterns_show.add_argument('--ai-only', action='store_true', help='Extract AI Quick Reference only')
    ...
    # Snippets command
    snippets_parser = subparsers.add_parser('snippets', help='Browse and search snippets')
    ...
    newtrack_id = newtrack_subparsers.add_parser('generate-id', help='Generate track ID')
    newtrack_id.add_argument('description', help='Track description')

    newtrack_scaffold = newtrack_subparsers.add_parser('scaffold', help='Create track directory structure')
    newtrack_scaffold.add_argument('track_id', help='Track ID')
    newtrack_scaffold.add_argument('--type', choices=['feature', 'bugfix', 'refactor', 'docs', 'chore'], default='feature')
    newtrack_scaffold.add_argument('--description', required=True, help='Track description')

    newtrack_register = newtrack_subparsers.add_parser('register', help='Register track in tracks.md')
    newtrack_register.add_argument('track_id', help='Track ID')
    newtrack_register.add_argument('--description', required=True, help='Track description')
    ...
    setup_templates = setup_subparsers.add_parser('copy-templates', help='Copy template files')
    setup_templates.add_argument('--languages', nargs='+', help='Languages to copy styleguides for')
    ...
    revert_commits = revert_subparsers.add_parser('find-commits', help='Find commits for a track')
    revert_commits.add_argument('track_id', help='Track ID')

    revert_plan = revert_subparsers.add_parser('plan-updates', help='Find plan file updates')
    revert_plan.add_argument('sha', help='Commit SHA')

    revert_list = revert_subparsers.add_parser('build-list', help='Build revert commit list')
    revert_list.add_argument('target', help='Target (track_id, phase, or task)')

    revert_exec = revert_subparsers.add_parser('execute', help='Execute revert sequence')
    revert_exec.add_argument('shas', nargs='+', help='Commit SHAs to revert')
    revert_exec.add_argument('--dry-run', action='store_true', help='Show what would be done')

    revert_subparsers.add_parser('parse-registry', help='Parse tracks registry for menu')
    ...
    impl_update = implement_subparsers.add_parser('update-status', help='Update track status')
    ...
    implement_subparsers.add_parser('modified-files', help='Get modified files from git')

    impl_coverage = implement_subparsers.add_parser('parse-coverage', help='Parse coverage report')
    impl_coverage.add_argument('--format', choices=['lcov', 'cobertura', 'json'], default='lcov')
    impl_coverage.add_argument('--path', help='Path to coverage file')

    impl_adr = implement_subparsers.add_parser('next-adr-number', help='Get next ADR number')
