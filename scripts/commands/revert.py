...
def build_list(project_root: Path, target: str) -> Dict[str, Any]:
    ...
    if ".." in target:
        ...
        code, stdout, _ = git_ops._run(
            ["log", "--format=%H", "--reverse", f"{start}..{end}"]
        )
    ...
    # Check if target is a single SHA (looks like hex)
        ...
        if full_sha:
            ...
            return {"success": False, "error": f"Commit '{target}' not found"}

    # Otherwise treat as track_id
    else:
        target_type = "track"
        if not resolver.track_exists(target):
            return {"success": False, "error": f"Track '{target}' not found"}
...
def execute(
    project_root: Path, shas: List[str], dry_run: bool = False
) -> Dict[str, Any]:
    """
    ...
    if not shas:
        return {"success": False, "error": "No SHAs provided"}
    ...
    for sha in shas:
        ...
        if not full_sha:
            return {"success": False, "error": f"Commit '{sha}' not found"}
    ...
    if dry_run:
        ...
        return {
            "success": True,
            "data": {"dry_run": True, "results": results, "count": len(results)},
...
def format_commits_list(commits: List[Dict], track_id: str) -> str:
    ...
    lines = [f"Commits for track: {track_id}", f"Total: {len(commits)}", ""]
...
def format_plan_updates(plan_files: List[str], sha: str) -> str:
    ...
    lines = [f"Plan files changed in {sha[:7]}:", ""]
...
def format_revert_list(commits: List[Dict], skipped: List[str], target: str) -> str:
    ...
    lines = [f"Revert plan for: {target}", ""]
...
def format_dry_run_results(results: List[Dict]) -> str:
    ...
    lines = ["Dry run - would perform the following reverts:", ""]
...
def format_registry_menu(
    in_progress: List[Dict], pending: List[Dict], completed: List[Dict]
) -> str:
    ...
    if completed:
        lines.append(
            f"{Formatters.status_symbol('completed')} Completed ({len(completed)} tracks)"
        )
