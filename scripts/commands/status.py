...
def verify_setup(project_root: Path) -> Dict[str, Any]:
    ...
    # Check each
    results = {"required": {}, "optional": {}, "directories": {}}

    missing_required = []

    for key, path in required_files.items():
        exists = resolver.exists(key)
        results["required"][key] = {"path": path, "exists": exists}
        if not exists:
            missing_required.append(key)

    for key, path in optional_files.items():
        exists = resolver.exists(key)
        results["optional"][key] = {"path": path, "exists": exists}

    for key, path in required_dirs.items():
        full_path = project_root / path
        exists = full_path.exists() and full_path.is_dir()
        results["directories"][key] = {"path": path, "exists": exists}
        if not exists:
            missing_required.append(key)
...
def calculate_progress(project_root: Path) -> Dict[str, Any]:
    ...
    if not tracks_dir or not tracks_dir.exists():
        return {"success": False, "error": "Tracks directory not found"}
    ...
    return {
        "success": True,
        "data": {"overall": overall, "by_track": track_metrics},
...
def full_status(project_root: Path) -> Dict[str, Any]:
    ...
    result = {
        ...
        "data": {
            ...
            "tracks": (
                tracks_result.get("data") if tracks_result.get("success") else None
            ),
    ...
    if git_status:
        ...
        if staged or modified or untracked:
            lines.append(
                f"Changes: {staged} staged, {modified} modified, {untracked} untracked"
            )
        else:
    ...
    if tracks_result.get("success"):
        ...
        lines.append(
            f"  {Formatters.status_symbol('completed')} Completed:   {summary.get('completed', 0)}"
        )
        lines.append(
            f"  {Formatters.status_symbol('in_progress')} In Progress: {summary.get('in_progress', 0)}"
        )
        lines.append(
            f"  {Formatters.status_symbol('pending')} Pending:     {summary.get('pending', 0)}"
        )
    lines.append("")
        ...
        if overall.get("total", 0) > 0:
            ...
            lines.append(
                Formatters.progress_bar(
                    overall.get("completed", 0), overall.get("total", 1)
                )
            )
            lines.append(
                f"  {overall.get('completed', 0)}/{overall.get('total', 0)} tasks completed"
            )
        ...
        for track in tracks:
            ...
            if tasks and tasks.get("total", 0) > 0:
                progress = Formatters.progress_bar(
                    tasks["completed"], tasks["total"], width=15
                )
